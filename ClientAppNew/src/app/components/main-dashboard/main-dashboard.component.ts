...
import { TestBenchService } from 'src/app/services/test-bench.service';
...
export class MainDashboardComponent implements OnDestroy {
 ...
 // type = "cylinder";
  ...
  constructor(private dashService: DashboardService, private testbenchService:TestBenchService,
    ...
    this.interval = setInterval((second)=>{
      ...
      this.initOilTank4()    
      this.getDashData();
      this.getPlcStatusData();
      this.getTestBenchRunningCount();
      this.getLast5TestBench();
      this.getObjectCount();
      this.getTestBenchData()
    },5000)
  }

 getDashData(){
  this.dashService.getDashData().pipe(
    map((data:any) => {
      if(data && data.dashData){
        return data.dashData.rows[0];
      } else {
        return []
      }
    })
 ...
 getPlcStatusData(){
  this.dashService.getPlcStatus().pipe(
    map((data:any) => {
      if(data && data.dashData){
        return data;
      } else {
 ...
 getTestBenchRunningCount(){
  this.dashService.getTestBenchRunningCount().pipe(
    map((data:any) => {
      if(data){
        return data;
      } else {
 ...
 getLast5TestBench(){
  this.dashService.getLast5TestBenchData().pipe(
    map((data:any) => {
      if(data){
        return data;
      } else {
 ...
 getObjectCount(){
  this.dashService.getObjectCount().pipe(
    map((data:any) => {
      if(data){
        return data;
      } else {
 ...
 getTestBenchData(){
  this.testbenchService.getTestBenchData().pipe(
    map((data:any) => {
      if(data){
        return data;
      } else {
  ...
  initEnvironMentCharts()
  {
    this.dashService.getDashData().pipe(
      ...
      this.dashData = data
        ...
        if (data) {
          data.dashData.rows.map((x,i)=>{
            
         data.dashData.rows[i].LabAirQualitySPM
         data.dashData.rows[i].LabAirQualityVOC
         data.dashData.rows[i].LabTemp
         data.dashData.rows[i].LabHumidity

          this.dataForSPM.push({ value:x.LabAirQualitySPM})
        
          this.dataForVoc.push({value:x.LabAirQualityVOC})
        
          this.dataTemp.push({value:x.LabTemp})
          
          this.dataHumidity.push({value:x.LabHumidity})
         
          })
          this.dataForSPM.reverse()
          this.dataForVoc.reverse()
          this.dataTemp.reverse()
          this.dataHumidity.reverse()
            // dataForSPM.push(LabAirQualitySPM)
            // console.log(dataForSPM,'sapm')
            // dataForSPM.reverse()
    
            // dataForVoc.push(LabAirQualityVOC)
            // dataForVoc.reverse()
           this.initChartAirQuality(this.dataForSPM , this.dataForVoc)
  
           // dataTemp.push(dataForTemperature)
    
           //dataTemp.reverse()
    
          this.initTemperatureChart(this.dataTemp);
    
           // dataHumidity.push(dataForhumidity)

    
          //dataHumidity.reverse()
          
          this.initHumidityChart(this.dataHumidity);
  ...
  initChartAirQuality(pumpValue, dpValue): void 
  {
    const data2 = {
      chart: {
        subcaption: '',
        ...
        //plottooltext: "<div style='font-size: 14px; padding: 10px;'><b>$seriesName</b>: $dataValue</div>",
        theme: "fusion"
      },
      categories: [
        {
